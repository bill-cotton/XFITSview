...
/* delete any characters string and buffer */
  if (TFilePtr->FileName) free(TFilePtr->FileName); TFilePtr->FileName=NULL;
  if (TFilePtr->directory) free(TFilePtr->directory); TFilePtr->directory=NULL;
...
/* delete structure */
  if (TFilePtr) free(TFilePtr); TFilePtr=NULL;
} /* end TextFileKill */
